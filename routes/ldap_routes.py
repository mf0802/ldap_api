...
def handle_create_user(payload: user.CreateUserPayload):
    # Create a new user object on the specified target domain
...
def handle_enable_user(payload: user.EnableUserPayload):
    # Set an AD-compliant password and enable the user account (Domain auto-detected from DN)
...
def handle_create_computer(payload: computer.CreateComputerPayload):
    # Create a new workstation computer object on the specified target domain
...
def handle_create_group(payload: group.CreateGroupPayload):
    # Create a new group object on the specified target domain
...
def handle_update_group_ownership(payload: group.UpdateGroupOwnershipPayload):
    # Update the group owner metadata inside the info block on a targeted domain
