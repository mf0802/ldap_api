...
class UpdateGroupOwnershipPayload(BaseModel):
    domain_name: str  # Added domain context for targeting correct forest
    group_name: str
...
class BatchGroupPayload(BaseModel):
    domain_name: str  # Added for cross-forest context
...
# =====================================================================
# MAIN DOMAIN-AWARE SERVICE HANDLERS
# =====================================================================
...
def create_group(payload: CreateGroupPayload) -> GroupCreationResponse:
    ...
    try:
        ...
        try:
            ...
            conn.unbind()  # Freeing connection resources reliably
...
def update_group_ownership(payload: UpdateGroupOwnershipPayload) -> GroupOwnershipResponse:
    """
    Update the Owners list inside the group's info attribute on the target domain.
    Returns a standardized API response model with the final info string.
    """
    try:
        # Fixed: Dynamic connection allocation with unpacked tuple
        conn, search_base = connect_to_domain(payload.domain_name)
        
        try:
            group_dn = find_dn(conn, payload.group_name, "group", search_base)
            ...
            if not conn.entries:
                raise AppError(
                    status_code=404, 
                    error="not_found", 
                    details=f"Group '{payload.group_name}' not found during read on forest."
                )
            ...
            return GroupOwnershipResponse(
                message=f"Group ownership for '{payload.group_name}' updated on '{payload.domain_name}'.",
...
def handle_batch(payload: BatchGroupPayload) -> BatchGroupResponse:
    """
    Batch add or remove user members to/from groups on a dynamically targeted domain.
    Returns a standardized API response model containing successful and failed operations.
    """
    try:
        # Validate action before allocating connection resources
        ...
        try:
            # 2. Iterate through groups and users
            for group_raw in payload.group_names:
                try:
                    group_dn = find_dn(conn, group_raw, "group", search_base)
                ...
                for user_raw in payload.user_names:
                    try:
                        user_dn = find_dn(conn, user_raw, "user", search_base)
            ...
            # 3. Clean up the socket connection once the batch loops are completed
            conn.unbind()
                    
        # Determine summary message based on the outcomes
        if not successful and failed:
            message = f"All batch operations failed on domain '{payload.domain_name}'."
        elif successful and failed:
            message = f"Batch operations completed with some errors on domain '{payload.domain_name}'."
        else:
            message = f"All batch operations completed successfully on domain '{payload.domain_name}'."
